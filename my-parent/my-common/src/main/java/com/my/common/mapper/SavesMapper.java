...
public interface SavesMapper<T> {
	...
	class SavesProvider extends BaseSelectProvider {
		...
		public String insert(MappedStatement ms) {
			...
			//先处理cache或bind节点
			for (EntityColumn column : columnList) {
				if (CreateMapper.CreateProvider.isSkip(column)) {
					continue;
				}
			...
			sql.append(CreateMapper.CreateProvider.insertColumns(entityClass, false, false, false));
			...
			for (EntityColumn column : columnList) {
				...
				if (StringUtil.isNotEmpty(column.getSequenceName())) {
					...
					sql.append(SqlHelper.getIfIsNull(entityName, column,
						column.getColumnHolder(entityName, "_bind", ","), isNotEmpty()));
				} else {
					//当null的时候，如果不指定jdbcType，oracle可能会报异常，指定VARCHAR不影响其他
					sql.append(SqlHelper.getIfIsNull(entityName, column, column.getColumnHolder(entityName, null, ","),
						isNotEmpty()));
				}
			}
			sql.append("</trim>");
